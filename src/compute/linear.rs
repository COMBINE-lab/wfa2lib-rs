...
pub fn compute_linear_idm(
    ...
    unsafe {
        let misms_ptr = wf_misms.offsets_centered_ptr();
        let gap_ptr = wf_gap.offsets_centered_ptr();
        let curr_ptr = wf_curr.offsets_centered_mut_ptr();
                ...
                unsafe {
                    ...
                    while k < avx_end {
                        let v_misms = _mm256_add_epi32(
                            _mm256_loadu_si256(misms_ptr.offset(k as isize) as *const __m256i),
                            v_one,
                        );
                        let v_ins = _mm256_add_epi32(
                            _mm256_loadu_si256(gap_ptr.offset(k as isize - 1) as *const __m256i),
                            v_one,
                        );
                        let v_del = _mm256_loadu_si256(gap_ptr.offset(k as isize + 1) as *const __m256i);
                        let v_max = _mm256_max_epi32(v_del, _mm256_max_epi32(v_ins, v_misms));
                        ...
                        _mm256_storeu_si256(curr_ptr.offset(k as isize) as *mut __m256i, v_result);
                        k += 8;
